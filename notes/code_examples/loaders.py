from datetime import datetime #It is used to record the start and end times of the data loading process.
from tqdm import tqdm #tqdm creates progress bars, which are useful for visualizing the progress of loops or long-running tasks
from datasets import load_dataset #used to easily download and work with various datasets, including those from Hugging Face.
from concurrent.futures import ProcessPoolExecutor, ThreadPoolExecutor #used for parallel processing, ProcessPoolExecutor uses multiple CPU processes, while ThreadPoolExecutor uses multiple threads.
...
class ItemLoader:
    ...
    def from_datapoint(self, datapoint):
        ...
        try:
            ...
            if price_str:
                price = float(price_str) # here we need to handle price as float otherwise will give error
                if MIN_PRICE <= price <= MAX_PRICE: #limiting this price range 
    ...
    def load(self, workers=8):
        ...
        should work on loading and scrubbing the data. NOTE: can be modified depending on the CPU power
        """
        start = datetime.now()
